...
wordList if wordList is not None else []


class shot:
    def __init__(self, frame_number, start_time, type=None, has_goal=None, has_goal_mouth=None, audio=None)
...
def main():
    ...
    shots.append((frame_numbers[-1], round((frame_times[-1]/60), 2), GoalDetector().execute(frames[int(
        last_cut/5)], frames[-1]), ShotClassifier(model_type=1).get_shot_class(frames), goalMouth(frames), False))
